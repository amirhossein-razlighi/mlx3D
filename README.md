...
- **Interactive viewer** — `mlx3d-view scene.ply` (or `.obj` / `.gltf` / `.glb`) auto-detects Gaussian checkpoints, meshes, and point clouds and opens a browser viewer with orbit/pan/zoom, a live control panel (background, exposure, clip plane, light, …), display-mode switching (RGB / depth / normals / wireframe), an orbit gizmo, and one-click PNG export. Frames are rendered on the Apple GPU by the Metal rasterizers and streamed live. Works for NeRFs too.
